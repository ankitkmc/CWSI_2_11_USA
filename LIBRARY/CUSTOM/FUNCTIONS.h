...
void neoway_publish(string topic) {
	...
	string jsonstring = data_packet.GET_JSON_STRING();
	if(SD_data!=NULL){
		strcpy(SD_data,jsonstring.c_str());
	}
...
#if defined(APP_CODE)
void neoway_publish_app(string topic) {
	neoway.SET_data_pub_topic(topic);
	neoway.SEND_RECIEVE("AT+AWSPUB=0,1,\"" + neoway.GET_data_pub_topic() + "\"," + to_string(strlen(encrypted_data)), { 5000 }, 1, { ">" });
	neoway.SEND_RECIEVE(encrypted_data, { 5000, 5000 }, 1, { "OK", "PUB" });
	restore_ble_print();
	data_packet.CLEAR();
